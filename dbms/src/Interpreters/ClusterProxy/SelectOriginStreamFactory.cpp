...
SelectOriginStreamFactory::SelectOriginStreamFactory(
    const Block & header,
...
BlockInputStreamPtr createLocalStream(const ASTPtr & query_ast, const Context & context, QueryProcessingStage::Enum processed_stage)
{
    ...
    InterpreterEnhanceJoinSelectQuery  interpreter{query_ast, context, {}, processed_stage, Protocol::Client::OriginPullQuery};
    BlockInputStreamPtr stream = interpreter.execute().in;
...
void SelectOriginStreamFactory::createForShard(
    ...
    BlockInputStreams & res)
{
    ...
    {
        LOG_DEBUG(&Logger::get("SelectOriginStreamFactory"),"add createLocalStream ,query :" + queryToString(query_ast));
    ...
    auto emplace_remote_stream = [&]()
    {
        ...
        LOG_DEBUG(&Logger::get("SelectOriginStreamFactory"),"add RemoteBlockInputStream ,query : " + query);

        auto stream = std::make_shared<RemoteBlockInputStream>(shard_info.pool, query, header, context,nullptr, throttler, external_tables, processed_stage, Protocol::Client::OriginPullQuery);
    ...
    {
        LOG_DEBUG(&Logger::get("SelectOriginStreamFactory"),"shard is local");
        ...
        if (!main_table_storage) /// Table is absent on a local server.
        {
            ...
            {
                LOG_WARNING(
                        &Logger::get("ClusterProxy::SelectOriginStreamFactory"),
        ...
        if (!replicated_storage)
        {
            ...
            LOG_DEBUG(&Logger::get("SelectOriginStreamFactory"),"no replicated_storage use local ");
            emplace_local_stream();
            return;
        }
        ...
        LOG_DEBUG(&Logger::get("SelectOriginStreamFactory"),"shard is remote ");
        emplace_remote_stream();
    }
