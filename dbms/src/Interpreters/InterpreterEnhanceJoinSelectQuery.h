...
/** Interprets the SELECT query. Returns the stream of blocks with the results of the query before `to_stage` stage.
  */
class InterpreterEnhanceJoinSelectQuery : public IInterpreter
{
public:
    /**
     * query_ptr
    ...
    InterpreterEnhanceJoinSelectQuery(
        const ASTPtr & query_ptr_,
        const Context & context_,
        const Names & required_result_column_names = Names{},
        QueryProcessingStage::Enum to_stage_ = QueryProcessingStage::Complete,
        const Protocol::Client::Enum query_type = Protocol::Client::ShuffleJoinMasterQuery,
        size_t subquery_depth_ = 0,
        const BlockInputStreamPtr & input = nullptr,
        bool only_analyze = false,
        std::shared_ptr<std::map<String,StoragePtr >> shuffle_tables = nullptr
       );
    ...
    static Block getSampleBlock(
        const ASTPtr & query_ptr_,
        const Context & context_,
        const Protocol::Client::Enum query_type,
...
private:
    struct Pipeline
    {
        ...
        void printStream(){
            ...
            int stream_index = 0;
            LOG_DEBUG(&Logger::get("InterpreterEnhanceJoinSelectQuery"),"start to print stream ");
            for(auto bs : streams){
                std::stringstream log_str;
                bs->dumpTree(log_str);
                LOG_DEBUG(&Logger::get("InterpreterEnhanceJoinSelectQuery"),"number " + std::to_string(stream_index) + " pipeline streams:\n"    + log_str.str() );
                stream_index ++;
            }
        }
    };
    ...
    InterpreterEnhanceJoinSelectQuery(
        ...
        const ASTPtr & query_ptr_,
        const Context & context_,
        const Protocol::Client::Enum query_type = Protocol::Client::Query,
        QueryProcessingStage::Enum to_stage = QueryProcessingStage::WithMergeableState,
    ...
    //void topLevelSecondStage(Pipeline & pipeline,AnalysisResult expressions );

    //void distributeSecondStage(Pipeline & pipeline,AnalysisResult expressions );
    ...
    struct AnalysisResult
    {
       ...
       // bool need_first_stage_proxy = false;
    ...
    //std::vector<IConnectionPool::Entry>  createConnections();
    void rewriteFirstStageQuery(const String & shuffleMainTable, const String & rightTable);
    void executeFirstStageProxy(Pipeline & pipeline,SubqueriesForSets & result);
    ...
    String  shuffle_main_table_name = "shuffle_main_table_01";
    String  shuffle_right_table_name = "shuffle_right_table_01";

    //std::shared_ptr<Cluster> cluster;


    //String shuffleMainTable;
